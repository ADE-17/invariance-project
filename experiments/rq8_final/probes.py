"""RQ8 frozen probes: logistic, tuned trees, RBF-SVM, three MLP capacities. Adapted from RQ5; held-out tuning only."""
...
def fit(ztrain, atrain, zval, aval, tests, folder, seed, device, smoke=False):
    ...
    tree_candidates=[]
    for leaves in ([15] if smoke else [15,31,63]):
        tree = HistGradientBoostingClassifier(max_iter=300 if not smoke else 20, max_leaf_nodes=leaves,
                l2_regularization=1, early_stopping=False, random_state=seed,class_weight='balanced').fit(x,atrain)
        tree_candidates.append((bce(aval,tree.predict_proba(v)[:,1]),tree))
    _,tree=min(tree_candidates,key=lambda pair:pair[0])
    joblib.dump(tree, folder / 'tree.joblib')
    audit['tree'] = dict(validation_balanced_bce=bce(aval, tree.predict_proba(v)[:, 1]),max_leaf_nodes=tree.max_leaf_nodes)
    rng=np.random.default_rng(seed)
    si=np.concatenate([rng.choice(np.flatnonzero(atrain==g),min(2000,int((atrain==g).sum())),replace=False) for g in [0,1]])
    svc_candidates=[]
    for c in ([1.] if smoke else [1.,10.]):
        svc=SVC(C=c,kernel='rbf',gamma='scale',class_weight='balanced',probability=True,random_state=seed,cache_size=512).fit(x[si],atrain[si])
        svc_candidates.append((bce(aval,svc.predict_proba(v)[:,1]),svc))
    _,svc=min(svc_candidates,key=lambda pair:pair[0]);joblib.dump(svc,folder/'rbf_svm.joblib')
    audit['rbf_svm']=dict(train_rows=len(si),C=svc.C,validation_balanced_bce=bce(aval,svc.predict_proba(v)[:,1]))
    for key in tests:
        result[key]['linear'] = linear.predict_proba(testx[key])[:, 1]
        result[key]['tree'] = tree.predict_proba(testx[key])[:, 1]
        result[key]['rbf_svm'] = svc.predict_proba(testx[key])[:, 1]
        result[key]['permuted_linear'] = negative.predict_proba(testx[key])[:, 1]
    for restart in range(3):
        torch.manual_seed(seed + 1000 + restart)
        dims = [[64], [256,128], [256,128,64]][restart]
        layers=[]; previous=x.shape[1]
        for width in dims:
            layers += [nn.Linear(previous,width),nn.ReLU()]; previous=width
        model = nn.Sequential(*layers,nn.Linear(previous,1),nn.Flatten(0)).to(device)
        ...
        model.eval()
        name = f'mlp{restart}'
        torch.save(dict(state=state, input_dim=x.shape[1], hidden_dims=dims, seed=seed+1000+restart, epoch=best_epoch), folder / f'{name}.pt')
