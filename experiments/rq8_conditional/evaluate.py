...
from experiments.rq8_conditional.common import *
from experiments.rq8_conditional.methods import grid
from experiments.rq8_conditional.calibration import diagnostics
from experiments.rq8_conditional.probes import fit
...
def evaluate(index,smoke=False):
    ...
    for s in tests:
        ...
        cal,cal_detail=diagnostics(y,p[s],a,d.unit.to_numpy()); metrics.update(cal)
        base_cal,base_cal_detail=diagnostics(y,base[s],a,d.unit.to_numpy()); erm.update(base_cal)
        row=dict(config=c,split=s,threshold=t,metrics=metrics,erm=erm,weak_calibration=cal_detail,erm_weak_calibration=base_cal_detail,threshold_05=extended(y,p[s],a,.5),
